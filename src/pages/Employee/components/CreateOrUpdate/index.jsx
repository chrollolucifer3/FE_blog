...
function CreateOrUpdate (props) {
  ...
  const handleConfirmCreateOrUpdateUser = () => {
    ...
    if (configModal.type !== "CREATE") {
      ...
      data.append(`confirmPassword`, dataCreateOrUpdate.confirmPassword);
    }
    ...
    if (!validate.isError) {
      if (configModal.type === "CREATE") {
        dispatch(handleCreateEmployee(dataCreateOrUpdate))
      } else {
        dispatch(handleUpdateEmployee(dataCreateOrUpdate, employee._id))
      }
    }
  }
